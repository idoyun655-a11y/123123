import { SimulationCore, SimulationStatus } from '../simulation/index.js';
...
export class AirportRuntime {
  ...
  initialize() {
    ...
    try {
      ...
      this.gateSystem = new GateSystem(this.simulationCore);
      this.runwaySystem = new RunwaySystem(this.simulationCore);
      ...
      this.groundSystem = new GroundHandlingSystem(this.simulationCore, { employeeSystem:this.employeeSystem, baggageSystem:this.baggageSystem, facilitySystem:this.facilitySystem });
      this.facilitySystem.attachGroundHandlingSystem(this.groundSystem);
  ...
  tick(realDeltaMs) { this.#requireInitialized(); const snap=this.simulationCore.tick(realDeltaMs); this.session.currentTime=new Date(snap.currentTime); this.session.currentDate=new Date(snap.currentTime); if (this.scheduler && snap.currentTime.getUTCHours()===0 && snap.currentTime.getUTCMinutes()<1) this.scheduler.generateDay(snap.currentTime); this.metrics.flights=this.flightSystem.getFlights().length; this.metrics.arrivals=this.flightSystem.getFlights().filter(f=>f.origin!=='ICN' && f.status!=='SCHEDULED').length; this.metrics.departures=this.flightSystem.getFlights().filter(f=>f.destination!=='ICN').length; this.metrics.delays=this.flightSystem.getFlights().filter(f=>f.delayMinutes>0).length; this.metrics.passengers=this.passengerSystem.getPassengers().length; this.metrics.baggage=this.baggageSystem.listBaggage().length; return snap; }
