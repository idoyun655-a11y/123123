...
import { AlertCenter } from '../ui/alerts.js';
...
class SeededRng { constructor(seed = 12345) { this.seed = (Number(seed) >>> 0) || 1; this.initialSeed = this.seed; } next() { let x=this.seed; x^=x<<13; x^=x>>>17; x^=x<<5; this.seed=x>>>0; return this.seed/0x100000000; } }
...
export class AirportRuntime {
  ...
  initialize() {
    ...
    try {
      ...
      this.eventEngine=new OperationalEventEngine({simulation:this.simulationCore}); this.alertCenter=new AlertCenter();
      this.weatherSystem=new WeatherSystem(this.simulationCore,{random:this.rng,alertCenter:this.alertCenter}); this.weatherSystem.initialize(this.options.startDate);
      ...
      this.gateSystem=new GateSystem(this.simulationCore); this.runwaySystem=new RunwaySystem(this.simulationCore,{weatherSystem:this.weatherSystem});
      ...
      this.groundSystem=new GroundHandlingSystem(this.simulationCore,{employeeSystem:this.employeeSystem,baggageSystem:this.baggageSystem,facilitySystem:this.facilitySystem,weatherSystem:this.weatherSystem}); this.facilitySystem.attachGroundHandlingSystem(this.groundSystem);
      this.flightSystem=new FlightSimulationSystem(this.simulationCore,{gateSystem:this.gateSystem,runwaySystem:this.runwaySystem,passengerSystem:this.passengerSystem,baggageSystem:this.baggageSystem,groundHandlingSystem:this.groundSystem,weatherSystem:this.weatherSystem});
