...
export class QueueEngine {
  ...
  createQueue({ id, serviceType, serverCount = 1, capacity = Infinity, discipline = QUEUE_DISCIPLINES.FIFO, serviceTime = 1, processor = null, zone = null }) {
    ...
    const queue = { id, serviceType, configuredServerCount: serverCount, serverCount, capacity, discipline, serviceTime, serviceRateMultiplier: 1, processor, zone, waiting: [], active: new Map(), completed: 0, totalServiceMinutes: 0, totalWaitMinutes: 0, maxWaitMinutes: 0, sequence: 0 };
    this.#queues.set(id, queue); return id;
  }
  updateServiceCapacity(queueId, { serverCount, serviceRateMultiplier = 1 } = {}) { const q = this.#require(queueId); if (!Number.isInteger(serverCount) || serverCount < 0) throw new RangeError('serverCount must be a non-negative integer.'); if (!Number.isFinite(serviceRateMultiplier) || serviceRateMultiplier < 0) throw new RangeError('serviceRateMultiplier must be non-negative.'); q.serverCount = serverCount; q.serviceRateMultiplier = serviceRateMultiplier; this.#dispatch(q, this.simulation.getSnapshot().currentTime); return this.stats(queueId); }
  setServerCount(queueId, serverCount) { return this.updateServiceCapacity(queueId, { serverCount, serviceRateMultiplier: this.#require(queueId).serviceRateMultiplier }); }
  ...
  stats(queueId) { const q = this.#require(queueId); const now = this.simulation.getSnapshot().currentTime; const waits = q.waiting.map((e) => Math.max(0, (now - e.enteredAt) / 60000)); const inSystem = q.waiting.length + q.active.size; return { id: q.id, queueLength: q.waiting.length, activeServers: q.active.size, serverCount: q.serverCount, configuredServerCount: q.configuredServerCount, throughput: q.completed, averageWaitingTime: q.completed ? q.totalWaitMinutes / q.completed : 0, maximumWaitingTime: Math.max(q.maxWaitMinutes, ...waits, 0), averageProcessingTime: q.completed ? q.totalServiceMinutes / q.completed : 0, utilization: q.serverCount ? Math.min(1, q.active.size / q.serverCount) : 0, serviceRateMultiplier: q.serviceRateMultiplier, inSystem }; }
  #dispatch(q, at) {
    while (q.active.size < q.serverCount && q.waiting.length) {
      const entry = q.waiting.shift(); const start = new Date(at); entry.exitTime = start; entry.passenger.startProcessing(start); const wait = Math.max(0, (start - entry.enteredAt) / 60000); q.totalWaitMinutes += wait; q.maxWaitMinutes = Math.max(q.maxWaitMinutes, wait); q.active.set(entry.id, entry);
      let serviceMinutes; try { serviceMinutes = resolveServiceTime(q.serviceTime, entry.passenger) / Math.max(q.serviceRateMultiplier, 0.000001); } catch (error) { q.active.delete(entry.id); q.waiting.unshift(entry); throw error; }
