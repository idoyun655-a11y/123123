...
export class GroundTask {
  constructor({ taskId, flightId, gateId = null, taskType, requiredStaff = 0, assignedEmployees = [], startTime = null, estimatedEndTime = null, actualEndTime = null, status = GROUND_TASK_STATUS.PENDING, priority = 0, dependencies = [], required = true } = {}) {
    if (!taskId || !flightId || !taskType) throw new TypeError('taskId, flightId and taskType are required.');
    this.taskId = taskId; this.flightId = flightId; this.gateId = gateId; this.taskType = taskType; this.requiredStaff = Math.max(0, Number(requiredStaff) || 0); this.assignedEmployees = [...assignedEmployees]; this.startTime = toDate(startTime); this.estimatedEndTime = toDate(estimatedEndTime); this.actualEndTime = toDate(actualEndTime); this.status = status; this.priority = Number(priority) || 0; this.dependencies = [...dependencies]; this.required = required; this.delayMinutes = 0; this.history = [];
  }
